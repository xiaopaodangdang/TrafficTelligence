...
var KEY_DATA = {
  filePath: "trae-inspector-file-path",
  startLine: "trae-inspector-start-line",
  startColumn: "trae-inspector-start-column",
  endLine: "trae-inspector-end-line",
  endColumn: "trae-inspector-end-column",
  staticProps: "trae-inspector-static-props"
};
...
function isTextOnlyChildrenForJsx(node) {
  return node.children.length > 0 && node.children.every((child) => {
    if ((0, import_types.isJSXText)(child)) {
    ...
    switch (type) {
            ...
            (node) => {
              if (node.type === 1) {
                if ((node.tagType === 0 || node.tagType === 1) && !EXCLUDE_TAG.includes(node.tag)) {
                  if (node.loc.source.includes(KEY_DATA.filePath))
                    return;
                  const staticProps = {};
                  const isChildrenTextOnly = isTextOnlyChildren(node);
                  let text = "";
                  if (isChildrenTextOnly) {
                    let startLine2, startColumn2, endLine2, endColumn2;
                    node.children.forEach((child, index) => {
                      if (child.type === 2) {
                        text += child.content;
                      }
                      if (index === 0) {
                        startLine2 = child.loc.start.line;
                        startColumn2 = child.loc.start.column;
                      }
                      if (index === node.children.length - 1) {
                        endLine2 = child.loc.end.line;
                        endColumn2 = child.loc.end.column;
                      }
                    });
                    staticProps.text = text.trim();
                    if (startLine2 && startColumn2) {
                      staticProps.textStartLine = startLine2.toString();
                      staticProps.textStartColumn = startColumn2.toString();
                    }
                    if (endLine2 && endColumn2) {
                      staticProps.textEndLine = endLine2.toString();
                      staticProps.textEndColumn = endColumn2.toString();
                    }
                  }
                  staticProps.cwd = process.cwd();
                  ...
                  const { line: startLine, column: startColumn } = node.loc.start;
                  const { line: endLine, column: endColumn } = node.loc.end;
                  const content = ` ${KEY_DATA.filePath}="${relativePath}" ${KEY_DATA.startLine}="${startLine}" ${KEY_DATA.startColumn}="${startColumn}" ${KEY_DATA.endLine}="${endLine}" ${KEY_DATA.endColumn}="${endColumn}" ${KEY_DATA.staticProps}="${encodeURIComponent(JSON.stringify(staticProps))}"`;
      ...
      case "jsx": {
        ...
        (0, import_core.traverse)(ast, {
          enter({ node }) {
            if (node.type === "JSXElement" && !EXCLUDE_TAG.includes(
              s.slice(
              ...
              if (node.openingElement.attributes.some(
                (attr) => attr.type !== "JSXSpreadAttribute" && attr.name.name === KEY_DATA.filePath
              ))
                return;
              ...
              const { line: endLine, column: endColumn } = node.loc.end;
              const staticProps = {};
              const isChildrenTextOnly = isTextOnlyChildrenForJsx(node);
              let text = "";
              if (isChildrenTextOnly) {
